defmodule MetaprogrammingElixir.TranslatorTest do
  use ExUnit.Case
  ...
  test "compile/1 generates catch-all t/3 functions" do
    assert MetaprogrammingElixir.Translator.compile([]) |> Macro.to_string() ==
             String.trim(~S"""
  ...
  test "compile/1 generates t/3 functions from each locale" do
    ...
    assert MetaprogrammingElixir.Translator.compile(locales) |> Macro.to_string() ==
             String.trim(~S"""
             def t(locale, path, bindings \\ [])
